...
TEST_P(CKKSEvaluatorTest, SignFunctionTest)
{
    vector<double> input = {-2.0, -1.0, -0.5, 0.0, 0.5, 1.0, 2.0};
    vector<double> expected = {-1.0, -1.0, -1.0, 0.0, 1.0, 1.0, 1.0};
    ...
    PhantomCiphertext result = evaluator->sgn_eval(cipher, 4, 4);

    evaluator->decryptor.decrypt(result, result_plain);
    ...
    {
        EXPECT_NEAR(expected[i], output[i], EPSILON);
    }
}

TEST_P(CKKSEvaluatorTest, InvertSqrtTest)
{
    vector<double> input = {1.0, 4.0, 9.0, 16.0};
    vector<double> expected = {1.0, 0.5, 1.0 / 3.0, 0.25};
    ...
    PhantomCiphertext result = evaluator->invert_sqrt(cipher);
...
TEST_P(CKKSEvaluatorTest, ExpFunctionTest)
{
    vector<double> input = {-2.0, -1.0, 0.0, 1.0, 2.0};
    vector<double> expected;
    for (double x : input)
    {
        expected.push_back(exp(x));
    }
    ...
    PhantomCiphertext result = evaluator->exp(cipher);
    ...
    for (size_t i = 0; i < expected.size(); i++)
    {
        EXPECT_NEAR(expected[i], output[i], EPSILON * 10);
    }
}

TEST_P(CKKSEvaluatorTest, InverseFunctionTest)
{
    vector<double> input = {1.0, 2.0, 4.0, 8.0};
    vector<double> expected = {1.0, 0.5, 0.25, 0.125};
    ...
    PhantomCiphertext result = evaluator->inverse(cipher);
